...
from corpus_preprocessing import clean
from corpus_preprocessing import meaningful
from corpus_preprocessing import corpus_properties
from corpus_preprocessing import save_positive_pairs
from corpus_preprocessing import load_positive_pairs
from skip_gram_negative_sampling import NeuralNetwork
import analysis
...
plt.show()
np.savez(f"saved_trained/trained_{n_lines}_{neg_per_pos}_{n_epochs}_{learning_rate}_.npz", word2idx=word2idx, idx2word=idx2word, word2vec=model.word2vec)

# model quality assess eval analysis
trained = np.load(f"saved_trained/trained_{n_lines}_{neg_per_pos}_{n_epochs}_{learning_rate}_.npz", allow_pickle=True)
