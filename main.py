...
from  dataset_loading import lazy_load_toy_dataset
...
word2idx, idx2word, idx2freq = corpus_properties(clean(lazy_load_dataset(n_lines = n_lines)))
# word2idx, idx2word, idx2freq = corpus_properties(clean(lazy_load_toy_dataset(n_lines = n_lines)))


# Compute skip-gram training pairs of (target_idx, context_idx) and persist to storage.
context_size = 2
save_positive_pairs(subsample(clean(lazy_load_dataset(n_lines = n_lines)), word2idx, idx2freq), word2idx, context_size)
# save_positive_pairs(clean(lazy_load_toy_dataset(n_lines = n_lines)), word2idx, context_size)

# Hyperparameters during training
vec_size = 50
neg_per_pos, n_max_epochs, learning_rate = 2, 100, 0.01
...
model = NeuralNetwork(word2idx, idx2word, idx2freq, vec_size)
