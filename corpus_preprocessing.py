...
def corpus_properties(sentence_generator):
    ...
    print()
    print(f"The vocabulary consists of {len(word2idx)} unique words for word2vec to learn the embedding of")
    print(f"The most occurring words are:")
    for i, (k, v) in enumerate(sorted(idx2occurences.items(), key= lambda t: t[1], reverse=True)):
        if i < 5:
...
def save_positive_pairs(
    sentence_generator, word2idx, context_size, save_file="positive_training_pairs.txt"
):
